...
PRIVATE void Insert( SBINTREE * ,  SBINTREE * );
PRIVATE void Delete( SBINTREE * ,  SBINTREE * );
PRIVATE void Search( SBINTREE * ,  SBINTREE * );
PRIVATE void UseFile( SBINTREE *,  SBINTREE *, BOOL);
...
void Menu( SBINTREE *BT , SBINTREE *AVL)
    {
    char cTmp[10];
    int iChoice;
    BOOL bLoop = TRUE;
    ...
    while (bLoop) 
    {
        ...
        if ( cTmp[0] != '\0' && (isdigit(*cTmp)) ) 
        {
            ...
            switch ( iChoice ) 
            {
                case 1: Insert(BT, AVL);            break;
                case 2: Delete(BT, AVL);            break;
                case 3: Search(BT, AVL);            break;
                case 4: UseFile(BT,AVL, TRUE);      break;
                case 5: UseFile(BT,AVL, FALSE);     break;
                case 6: Display(BT);
                        Display(AVL);               break;
                case 7: SaveLog(BT);
                        SaveLog(AVL);               break;
...
PRIVATE void Insert( SBINTREE *BT, SBINTREE *AVL )
{
...
PRIVATE void Delete( SBINTREE *BT, SBINTREE *AVL )
{
    char strWord[STR_SIZE] ;
...
PRIVATE void Search( SBINTREE *BT, SBINTREE *AVL )
{
    char strWord[STR_SIZE] ;
...
PRIVATE void UseFile( SBINTREE *BT,  SBINTREE *AVL, BOOL bIns )
{
