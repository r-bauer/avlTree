...
*/
PRIVATE int Balance(SLINK slkNode)
{
    if (slkNode)
        ...
        else 
        {
            ...
            if (iCmp == 0)      // noh duplicado
            {
                ...
                { 
                    ...
                    slkNode->sLink[RIGHT] = RemoveNodeAvl(BST, slkNode->sLink[RIGHT], slkFind);
                }
            }
            else        // noh maior ou menor
            {
                // avanca a posicaoh 
                slkNode->sLink[iDir] = RemoveNodeAvl(BST, slkNode->sLink[iDir], slkFind);
            }

            if (slkNode)
            {
                // atualiza altura
                slkNode->iHeight = Higher( Height(slkNode->sLink[RIGHT]), Height(slkNode->sLink[LEFT]) ) + 1;
...
void DelNodeAvl(SBINTREE *BST, void *pData)
{
    ...
    {
        BSTRoot->sLink[RIGHT] = RemoveNodeAvl(BST, BSTRoot->sLink[RIGHT], slkFind);
    }
}
